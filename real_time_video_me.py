...
class Emotion_Rec:
    ...
    def run(self, frame_in, canvas, label_face, label_result):
        ...
        # 调节画面大小
        frame = imutils.resize(frame_in, width=300)  # 缩放画面，针对图像处理的库，调整图片大小
        ...
        #OpenCV库中自带的人脸识别模型，该方法为滑窗式，第一个参数为图像，第二个为按一定比例缩小图片或扩大滑窗的参数，参数越大，计算计算速度越快，越容易出错，第三个为构成检测目标的相邻矩形的最小个数，意义为数值越大，对人脸检测的准确度越高，既有邻居才能被识别，反之则不可以，依赖旁边的邻居，
        preds = []  # 预测的结果
        label = None  # 预测的标签
